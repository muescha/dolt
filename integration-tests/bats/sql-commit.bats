...
@test "sql-commit: DOLT_COMMIT with all flag, message and author" {
    skip("will fix before merge")
    run dolt sql -r csv -q "call dolt_commit('-a', '-m', 'Commit1', '--author', 'John Doe <john@doe.com>') as commit_hash"
    [ $status -eq 0 ]
    DCOMMIT=$output
...
@test "sql-commit: DOLT_COMMIT works with --author without config variables set" {
    ...
    run dolt sql -q "call dolt_commit('-m', 'Commit1', '--author', 'John Doe <john@doe.com>') as commit_hash"
    [ "$status" -eq 0 ]
    DCOMMIT=$output
